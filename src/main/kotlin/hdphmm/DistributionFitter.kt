...
class GaussianDistributionFitter1D {
    companion object: DistributionFitter<Double, GaussianDistribution1DParameters> {
...
class PoissonDistributionFitter {
    companion object: DistributionFitter<Double, PoissonDistributionParameters> {
        ...
        override fun fitParameters(samples: Iterable<Double>): PoissonDistributionParameters =
                PoissonDistributionParameters(estimateRate(samples.toDoubleArray1D()))
    }
}

data class LaplaceDistributionParameters(val mean: Double, val scale: Double)

class LaplaceDistributionFitter {
    companion object: DistributionFitter<Double, LaplaceDistributionParameters> {
...
class BetaDistributionFitter {
    companion object: DistributionFitter<Double,BetaDistributionParameters> {
        override fun fitParameters(samples: Iterable<Double>): BetaDistributionParameters {
            val mean = samples.average()
            val std = samples.toDoubleArray1D().std()

            return BetaDistributionParameters(alpha = estimateAlpha(mean, std), beta = estimateBeta(mean, std))
        }
...
class GeometricDistributionFitter {
    companion object: DistributionFitter<Double,GeometricDistributionParameters> {
        override fun fitParameters(samples: Iterable<Double>): GeometricDistributionParameters =
                GeometricDistributionParameters(estimateSuccessProbability(samples.toDoubleArray1D()))
...
class ExponentialDistributionFitter {
    companion object: DistributionFitter<Double,ExponentialDistributionParameters> {
        override fun fitParameters(samples: Iterable<Double>) =
                ExponentialDistributionParameters(estimateDecayRate(samples.toDoubleArray1D()))
...
class GammaDistributionFitter {
    companion object: DistributionFitter<Double,GammaDistributionParameters> {
...
class ParetoDistributionFitter {
    companion object: DistributionFitter<Double,ParetoDistributionParameters> {
        override fun fitParameters(samples: Iterable<Double>): ParetoDistributionParameters =
                samples.toDoubleArray1D().let {
                    ParetoDistributionParameters(
                            scale = estimateScale(it),
                            alpha = estimateAlpha(it, estimateScale(it))
                    )
                }
